...
tau_lyseries = lambda tau, oscillator_strength, transition_wavelength: (
    tau
    * oscillator_strength
...
class QSOLoader(object):
    ...
    def __init__(
        self,
        preloaded_file="preloaded_qsos.mat",
        catalogue_file="catalog.mat",
        learned_file="learned_qso_model_dr9q_minus_concordance.mat",
        processed_file="processed_qsos_dr12q.mat",
        dla_concordance="dla_catalog",
        los_concordance="los_catalog",
        snrs_file="snrs_qsos_dr12q.mat",
        sub_dla=True,
        ...
        self.test_ind = self.processed_file["test_ind"][0, :].astype(
            np.bool
        )  # size: (num_qsos, )
    ...
    def reevaluate_model_posteriors(self):
        ...
        if np.where(self.model_posteriors.sum(axis=1) > 1.2)[0].shape[0] != 0:
            ...
            log_posteriors = np.concatenate(
                (log_posteriors_no_dla, log_posteriors_lls, log_posteriors_dla), axis=1
            )

            max_log_posteriors = log_posteriors.max(axis=1)

            model_posteriors = np.exp(log_posteriors - max_log_posteriors[:, None])
    ...
    def prepare_roman_map_vals(
        self, sample_file="dla_samples.mat", use_memory=False, split=10
    ):
        """
        ...
        if "sample_log_likelihoods_dla" not in self.processed_file.keys():
            raise Exception(
                KeyError, "Should use the processed file with sample likelihoods."
            )
        ...
        if use_memory:
            ...
            for init_nspec, end_nspec in zip(
                range(0, length, chunk), range(chunk, length + chunk, chunk)
            ):
                ...
                likelihoods = self.processed_file["sample_log_likelihoods_dla"][
                    :, real_index_slice
                ]
                ...
                sample_z_dlas = (
                    self.min_z_dlas[init_nspec:end_nspec]
                    + (
    ...
    def load_dla_concordance(
        self, dla_concordance, los_concordance, lnhi_min=20, release="dr9"
    ):
        """
    ...
    def load_dla_parks(
        self, dla_parks, p_thresh=0.5, release="dr12", multi_dla=True, num_dla=2
    ):
        """
    ...
    def make_multi_unique_id(self, num_dla, plates, mjds, fiber_ids):
        ...
        """
        multi_unique_id = np.ones(num_dla)[:, None] * self.make_unique_id(
            plates, mjds, fiber_ids
        )
    ...
    def make_multi_ROC(self, catalog):
        ...
        dla_ind = np.in1d(
            catalog.multi_real_index_los, catalog.multi_real_index_dla
        )  # shape : flatten( (num_dla, num_los) )
        ...
        for odd in odds_dla_no_dla:
            ...
            FPR.append(
                ...
                / (np.sum(false_positives) + np.sum(true_negatives))
            )
    ...
    def make_ROC(self, catalog, occams_razor=10000):
        ...
        """
        dla_ind = np.in1d(
            catalog.real_index_los, catalog.real_index
        )  # boolean array, same size
        ...
        log_posteriors_dla = self.processed_file["log_posteriors_dla"][()] - np.log(
            occams_razor
        )
        log_posteriors_no_dla = self.processed_file["log_posteriors_no_dla"][0, :]
        if self.sub_dla:
            log_posteriors_lls = self.processed_file["log_posteriors_lls"][
                0, :
            ] - np.log(occams_razor)

            # update no DLA posteriors with subDLA posteriors
            log_posteriors_no_dla = logsumexp(
                [log_posteriors_no_dla, log_posteriors_lls], axis=0
            )
    ...
    def make_MAP_comparison(self, catalog):
        ...
        map_z_dlas = self.map_z_dlas[
            real_index, 0, 0
        ]  # assume DLA(1) corresponds to the concordance value
        map_log_nhis = self.map_log_nhis[real_index, 0, 0]

        Delta_z_dlas = (
            map_z_dlas - self.dla_catalog.z_dlas[map_model_index > self.sub_dla]
        )
        Delta_log_nhis = (
            map_log_nhis - self.dla_catalog.log_nhis[map_model_index > self.sub_dla]
        )
    ...
    def make_MAP_parks_comparison(
        self, catalog, num_dlas=1, dla_confidence=0.98, return_map_values: bool = False
    ):
        """
        ...
        for i, uid in enumerate(uids_dla_n):
            ...
            assert np.all(
                np.abs(
                    (
                        this_z_dlas[argsort_garnett] - this_z_dlas_parks[argsort_parks]
                    ).sum()
                )
            ...
            Delta_log_nhis[i, :] = (
                this_log_nhis[argsort_garnett] - this_log_nhis_parks[argsort_parks]
            )
    ...
    def make_multi_confusion(
        ...
        for i, uid in enumerate(unique_ids):
            ...
            if n > 0:
                inds = (this_map_z_dlas[n - 1, :] > min_z_dla) * (
                    this_map_log_nhis[n - 1, :] > min_log_nhi
                )
                n = np.sum(inds)
            ...
            m = self.query_least_num_dlas_parks(
                uid,
                catalog.raw_unique_ids,
                catalog.raw_dla_confidences,
                catalog.raw_z_dlas,
                catalog.raw_log_nhis,
                dla_confidence,
                min_z_dla,
                min_log_nhi,
            )
    ...
    @staticmethod
    def prediction_json2dict(
        filename_parks="predictions_DR12.json", object_name="dlas"
    ):
        """
    ...
    def column_density_function_parks(
        ...
        **kwargs
        ...
        (
            ...
            all_snrs,
            p_dlas,
        ) = self._get_parks_estimations(dla_parks, p_thresh=p_thresh, **kwargs)
        ...
        if apply_p_dlas:
            tot_f_N, NHI_table = np.histogram(
                10 ** log_nhis, 10 ** lnhis, weights=p_dlas
            )
        else:
        ...
        dN = np.array(
            [
                10 ** lnhi_x - 10 ** lnhi_m
                for (lnhi_m, lnhi_x) in zip(lnhis[:-1], lnhis[1:])
            ]
        )
        ...
        l_Ncent = np.array(
            [(lnhi_x + lnhi_m) / 2.0 for (lnhi_m, lnhi_x) in zip(lnhis[:-1], lnhis[1:])]
        )
    ...
    def line_density_park(
        ...
        **kwargs
        ...
        dX = np.array(
            [
                self.path_length(min_z_dlas, max_z_dlas, z_m, z_x)
    ...
    def omega_dla_parks(
        ...
        **kwargs
        ...
        # ref: https://github.com/sbird/fake_spectra/blob/master/fake_spectra/spectra.py#L976
        if apply_p_dlas:
            nhi_tot_f_N, NHI_table = np.histogram(
                z_dlas, z_bins, weights=weight * p_dlas
            )
        else:
    ...
    def plot_omega_dla_parks(
        ...
        **kwargs
        ...
        (z_cent, omega_DLA, xerrs) = self.omega_dla_parks(
            dla_parks,
            z_min=zmin,
            z_max=zmax,
            hubble=hubble,
    ...
    def column_density_function_noterdaeme(
        ...
        (
            ...
            snrs,
            all_snrs,
        ) = self._get_noterdaeme_estimations(dla_noterdaeme, los_noterdaeme)
    ...
    def _get_noterdaeme_estimations(self, dla_noterdaeme, los_noterdaeme):
        ...
        min_z_dlas = (1 + z_qsos) * (
            lyb_wavelength + kms_to_z(3000)
        ) / lya_wavelength - 1
        max_z_dlas = (1 + z_qsos) * (
            lya_wavelength - kms_to_z(5000)
        ) / lya_wavelength - 1
    ...
    def find_this_wavelengths(self, nspec):
        ...
        return self.preloaded_file[
            self.preloaded_file["all_wavelengths"][0, nspec_real]
        ][0]
    ...
    def find_this_noise_variance(self, nspec):
        ...
        return self.preloaded_file[
            self.preloaded_file["all_noise_variance"][0, nspec_real]
        ][0]
    ...
    def find_this_pixel_mask(self, nspec):
        ...
        return self.preloaded_file[
            self.preloaded_file["all_pixel_mask"][0, nspec_real]
        ][0].astype(np.bool_)
    ...
    def plot_mean_flux(self, nspec, suppressed=True, num_lines=31):
        ...
        plt.plot(
            self.GP.rest_wavelengths,
            this_mu,
            label=r"mean-flux $\mu \circ exp(-\tau (1+z)^\beta)$",
            color="red",
        )
    ...
    def plot_this_mu(
        ...
        if Parks:
            plt.plot(
                rest_wavelengths,
                this_parks_mu,
                label=r"Parks: z_dlas = ({}); lognhis=({}); p_dlas=({})".format(
    ...
    def total_scale_factor(tau, beta, z_qso, rest_wavelengths, num_lines=31):
        ...
        for i in range(num_lines):
            ...
            this_lyman_1pzs = (
                rest_wavelengths * (1 + z_qso) / all_transition_wavelengths[i]
            )
    ...
    def plot_raw_spectrum(self, nspec, release="dr12q", download=True):
        ...
        if not os.path.exists(filename):
            dirname = "{}/{:d}".format(spectra_directory(release), self.plates[nspec])
        ...
        ax.plot(
            wavelengths, noise_variance, lw=0.25, label=r"$\sigma^2(\lambda_{obs})$"
        )
        ax.set_xlabel(r"observed wavelength [$\AA$]")
        ax.set_ylabel(r"flux [$10^{-17}{erg}/s/{cm}^{2}/\AA$]")
        ax.set_title(r"{} at z_qso = {:.3g}".format(filename, self.z_qsos[nspec]))
...
file_loader = (
    lambda release, plate, mjd, fiber_id: "{}/{:d}/spec-{:d}-{:d}-{:04d}.fits".format(
...
def conditional_mvnpdf_low_rank(
    ...
    K22_inv_Sigma21 = d2_inv * np.matmul(M2, M1.T) - np.matmul(
        D2_inv_M2, np.matmul(C2, np.matmul(M2, M1.T))
    )
    ...
    Sigma11 = (
        np.diag(d1)
        + np.matmul(M1, M1.T)
        - np.matmul(np.matmul(M1, M2.T), K22_inv_Sigma21)
    )
